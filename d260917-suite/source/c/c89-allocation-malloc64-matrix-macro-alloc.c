// <svf-test-suite>@64f017e/src/mem_leak/malloc64.c
...
static void tp_init_vogel (int nrow, float *row, int ncol, float *col, float **cost, struct sol ***_sol)
{
	...
	struct sol **sol;
	r_del = calloc(nrow, sizeof(*r_del));
	c_del = calloc(ncol, sizeof(*c_del));
...
int foo(){
	...
	float *cost[1] = { row };
	struct sol **sol;
	tp_init_vogel(1, row, 1, col, cost, &sol);
	return 0;
}
int main(){
 return foo();
