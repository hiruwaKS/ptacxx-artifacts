// <svf-test-suite>@64f017e/src/basic_c_tests/spec-equake.c
  ...
  double sum0, sum1, sum2, sum3;
	...
	disp = (double ***) malloc( 3*sizeof(double**));
        for(disptplus = 0; disptplus < 3; disptplus++){
           disp[disptplus] = (double **) malloc( 1000*sizeof(double*));
           for(i = 0; i < 1000; i++)
	...
	K = (double ***) malloc( 3*sizeof(double**));
        for(disptplus = 0; disptplus < 3; disptplus++){
           K[disptplus] = (double **) malloc( 1000*sizeof(double*));
           for(i = 0; i < 1000; i++)
	...
	{printf("null pointer 2!\n"); return 0;}
        ...
        v = (double **) malloc( 1000*sizeof(double*));
        for(i = 0; i < 1000; i++)
           v[i] = (double *) malloc( 4*sizeof(double));
	if(v[999] == NULL)
	{printf("null pointer 2!\n"); return 0;}

        ARCHmatrixindex = (int *) malloc( 1002*sizeof(int));
